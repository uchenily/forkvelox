...
RowVectorPtr makeBatch(memory::MemoryPool *pool, const std::vector<int64_t> &values) {
  ...
  auto vec = std::make_shared<FlatVector<int64_t>>(pool, BIGINT(), nullptr, values.size(), buffer);
  return std::make_shared<RowVector>(pool, ROW({"my_col"}, {BIGINT()}), nullptr, values.size(),
                                     std::vector<VectorPtr>{vec});
}

RowVectorPtr makeKeyValueBatch(memory::MemoryPool *pool, const std::vector<int64_t> &keys,
  ...
  auto keyVec = std::make_shared<FlatVector<int64_t>>(pool, BIGINT(), nullptr, keys.size(), keyBuffer);
  auto valueVec = std::make_shared<FlatVector<int64_t>>(pool, BIGINT(), nullptr, values.size(), valueBuffer);
  return std::make_shared<RowVector>(pool, ROW({"k", "v"}, {BIGINT(), BIGINT()}), nullptr, keys.size(),
...
int main(int argc, char **argv) {
  ...
  std::vector<RowVectorPtr> groupedPartialBatches{groupedBatch1, groupedBatch2, groupedBatch3};
