...
class FilterLambdaExample : public VectorTestBase {
public:
  FilterLambdaExample() {
  ...
  std::shared_ptr<ArrayVector> makeArrayVector(const std::vector<std::vector<int64_t>> &data) {
    ...
    auto offsetsBuffer = AlignedBuffer::allocate(rowCount * sizeof(int32_t), pool());
    auto sizesBuffer = AlignedBuffer::allocate(rowCount * sizeof(int32_t), pool());
    ...
    return std::make_shared<ArrayVector>(pool(), ARRAY(BIGINT()), nullptr, rowCount, offsetsBuffer, sizesBuffer,
                                         elementsVector);
  }
  ...
  void run() {
    ...
    std::cout << std::endl << "> input rows (a: array(bigint), b: bigint):" << std::endl;
    std::cout << data->toString() << std::endl;

    // Lambda body must be wrapped in parentheses: x -> (x % 2 == 0).
    auto evenExpr = compileExpression("filter(a, x -> (x % 2 == 0))", asRowType(data->type()));
    auto evens = evaluate(*evenExpr, data);

    std::cout << std::endl << "> filter(a, x -> (x % 2 == 0)):" << std::endl;
    std::cout << evens->toString() << std::endl;

    auto captureExpr = compileExpression("filter(a, x -> (x >= b))", asRowType(data->type()));
    auto captured = evaluate(*captureExpr, data);

    std::cout << std::endl << "> filter(a, x -> (x >= b)):" << std::endl;
    std::cout << captured->toString() << std::endl;
  }
};
