...
public:
  ...
  std::vector<std::shared_ptr<const core::PlanNode>> sources() const override {
    return {};
  }
...
public:
  SlowValuesOperator(core::PlanNodePtr node, std::vector<RowVectorPtr> values,
                     std::chrono::milliseconds sleepPerBatch)
      : Operator(std::move(node)), values_(std::move(values)),
        sleepPerBatch_(sleepPerBatch) {}
...
std::vector<RowVectorPtr> makeBatches(memory::MemoryPool *pool,
                                      vector_size_t batchSize, int numBatches) {
...
int main(int argc, char **argv) {
  ...
  auto slowOp = std::make_shared<SlowValuesOperator>(
      planNode, batches, std::chrono::milliseconds(2));
  ...
  while (true) {
    ...
    if (reason == BlockingReason::kYield) {
      ++yields;
      std::cout << "[Demo] Requeue after yield (" << yields << ")."
                << std::endl;
      continue;
    }
    if (reason == BlockingReason::kCancelled) {
      std::cout << "[Demo] Cancelled after " << results.size() << " batches."
                << std::endl;
      break;
    }
    if (reason == BlockingReason::kNotBlocked) {
      std::cout << "[Demo] Completed with " << results.size() << " batches."
                << std::endl;
      break;
    }
    std::cout << "[Demo] Stopped with reason " << static_cast<int>(reason)
              << "." << std::endl;
    break;
  }
