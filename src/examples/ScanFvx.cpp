...
RowVectorPtr makeSampleData(memory::MemoryPool* pool) {
  ...
  auto ids =
      AlignedBuffer::allocate(numRows * sizeof(int64_t), pool);
  auto* rawIds = ids->asMutable<int64_t>();
  for (vector_size_t i = 0; i < numRows; ++i) {
    rawIds[i] = i + 1;
  }
  auto idVector =
      std::make_shared<FlatVector<int64_t>>(pool, BIGINT(), nullptr, numRows, ids);
  ...
  auto prices =
      AlignedBuffer::allocate(numRows * sizeof(int32_t), pool);
  ...
  return std::make_shared<RowVector>(
      pool,
      ROW({"id", "label", "price"}, {BIGINT(), VARCHAR(), INTEGER()}),
      nullptr,
      numRows,
