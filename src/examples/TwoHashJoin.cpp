...
std::shared_ptr<FlatVector<int64_t>> makeInt64Vector(
  ...
  return std::make_shared<FlatVector<int64_t>>(
      pool, BIGINT(), nullptr, data.size(), buffer);
}

std::shared_ptr<FlatVector<StringView>> makeStringVector(
    memory::MemoryPool* pool,
  ...
  auto vec = std::make_shared<FlatVector<StringView>>(
      pool, VARCHAR(), nullptr, size, values);
...
RowVectorPtr makeRowVector(
  ...
  return std::make_shared<RowVector>(
      pool, ROW(names, types), nullptr, children[0]->size(), children);
}
