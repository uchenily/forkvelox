...
makeInt64Vector(memory::MemoryPool *pool, const std::vector<int64_t> &data) {
  auto buffer = AlignedBuffer::allocate(data.size() * sizeof(int64_t), pool);
  std::memcpy(buffer->asMutable<uint8_t>(), data.data(),
              data.size() * sizeof(int64_t));
  return std::make_shared<FlatVector<int64_t>>(pool, BIGINT(), nullptr,
                                               data.size(), buffer);
}

std::shared_ptr<FlatVector<StringView>>
makeStringVector(memory::MemoryPool *pool,
  ...
  auto vec = std::make_shared<FlatVector<StringView>>(pool, VARCHAR(), nullptr,
                                                      size, values);
...
RowVectorPtr makeRowVector(memory::MemoryPool *pool,
  ...
  return std::make_shared<RowVector>(pool, ROW(names, types), nullptr,
...
int main(int argc, char **argv) {
  ...
  auto probeBatch =
      makeRowVector(pool.get(), {"key"}, {BIGINT()},
                    {makeInt64Vector(pool.get(), {0, 1, 2, 3, 4})});

  auto buildBatch1 =
      makeRowVector(pool.get(), {"key", "name"}, {BIGINT(), VARCHAR()},
                    {makeInt64Vector(pool.get(), {0, 1, 2, 3}),
                     makeStringVector(pool.get(), {"R0", "R1", "R2", "R3"})});
