...
VectorPtr makeRepeatedVector(const VectorPtr &source, vector_size_t sourceRow,
                             vector_size_t size, memory::MemoryPool *pool) {
  if (auto simple64 =
          std::dynamic_pointer_cast<SimpleVector<int64_t>>(source)) {
    auto buffer = AlignedBuffer::allocate(size * sizeof(int64_t), pool);
    auto flat = std::make_shared<FlatVector<int64_t>>(pool, source->type(),
                                                      nullptr, size, buffer);
  ...
  if (auto simple32 =
          std::dynamic_pointer_cast<SimpleVector<int32_t>>(source)) {
    auto buffer = AlignedBuffer::allocate(size * sizeof(int32_t), pool);
    auto flat = std::make_shared<FlatVector<int32_t>>(pool, source->type(),
                                                      nullptr, size, buffer);
    auto *raw = flat->mutableRawValues();
  ...
  if (auto simpleStr =
          std::dynamic_pointer_cast<SimpleVector<StringView>>(source)) {
    auto buffer = AlignedBuffer::allocate(size * sizeof(StringView), pool);
    auto flat = std::make_shared<FlatVector<StringView>>(pool, source->type(),
                                                         nullptr, size, buffer);
    auto *raw = flat->mutableRawValues();
...
public:
  LambdaCallable(RowTypePtr signature, ExprPtr body,
                 std::vector<std::string> captureNames,
                 const RowVector *outerRow, core::ExecCtx *execCtx)
      : signature_(std::move(signature)), body_(std::move(body)),
        captureNames_(std::move(captureNames)), outerRow_(outerRow),
        execCtx_(execCtx) {
    auto outerType = asRowType(outerRow_->type());
    VELOX_CHECK_NOT_NULL(outerType.get(),
                         "Lambda evaluation requires a RowVector input");
  ...
  void apply(vector_size_t topLevelRow, const SelectivityVector &rows,
             EvalCtx &context, const std::vector<VectorPtr> &args,
             VectorPtr &result) override {
    VELOX_CHECK_EQ(args.size(), signature_->size(),
    ...
    for (size_t i = 0; i < captureIndices_.size(); ++i) {
      ...
      children.push_back(makeRepeatedVector(captureVector, topLevelRow,
                                            rows.size(), execCtx_->pool()));
    }

    auto lambdaRow =
        std::make_shared<RowVector>(execCtx_->pool(), lambdaRowType_, nullptr,
...
void LambdaExpr::eval(const SelectivityVector &rows, EvalCtx &context,
                      VectorPtr &result) {
  VELOX_CHECK_NOT_NULL(context.row(),
                       "Lambda evaluation requires a RowVector input");
  auto functions =
      std::make_shared<FunctionVector>(context.pool(), type_, rows.size());
  auto callable = std::make_shared<LambdaCallable>(
      signature_, body_, captures_, context.row(), context.execCtx());
