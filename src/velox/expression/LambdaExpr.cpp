...
VectorPtr makeRepeatedVector(const VectorPtr &source, vector_size_t sourceRow, vector_size_t size,
...
public:
  LambdaCallable(RowTypePtr signature, ExprPtr body, std::vector<std::string> captureNames, const RowVector *outerRow,
                 core::ExecCtx *execCtx)
      : signature_(std::move(signature)), body_(std::move(body)), captureNames_(std::move(captureNames)),
        outerRow_(outerRow), execCtx_(execCtx) {
    auto outerType = asRowType(outerRow_->type());
    VELOX_CHECK_NOT_NULL(outerType.get(), "Lambda evaluation requires a RowVector input");
  ...
  void apply(vector_size_t topLevelRow, const SelectivityVector &rows, EvalCtx &context,
             const std::vector<VectorPtr> &args, VectorPtr &result) override {
    ...
    auto lambdaRow =
        std::make_shared<RowVector>(execCtx_->pool(), lambdaRowType_, nullptr, rows.size(), std::move(children));
