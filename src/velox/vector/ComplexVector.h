...
public:
    ArrayVector(memory::MemoryPool* pool,
                ...
                BufferPtr sizes,
                VectorPtr elements)
        : BaseVector(pool, type, VectorEncoding::Simple::ARRAY, std::move(nulls), length),
    ...
    std::string toString(vector_size_t index) const override {
        ...
        return stringifyTruncatedElementList(
            size,
            [this, offset](std::stringstream& out, size_t i) {
    ...
    int32_t compare(const BaseVector* other, vector_size_t index, vector_size_t otherIndex) const override {
        ...
        for (int32_t i = 0; i < minSize; ++i) {
            int32_t cmp = elements_->compare(
                otherArray->elements_.get(),
                offsetAt(index) + i,
