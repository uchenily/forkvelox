...
[[noreturn]] void veloxCheckFail(const char *file, int line, const char *expr,
                                 std::format_string<Args...> fmt,
                                 Args &&...args) {
  std::string msg = std::format(fmt, std::forward<Args>(args)...);
  std::string fullMsg =
      std::format("[{}:{}] Check failed: `{}` {}", file, line, expr, msg);
  throw VeloxRuntimeError(fullMsg);
}

// Overload for no format args
[[noreturn]] inline void veloxCheckFail(const char *file, int line,
...
#define VELOX_CHECK(condition, ...)                                            \
  do {                                                                         \
    if (!(condition)) {                                                        \
      ::facebook::velox::detail::veloxCheckFail(__FILE__, __LINE__,            \
                                                #condition, __VA_ARGS__);      \
    }                                                                          \
  } while (0)

#define VELOX_CHECK_EQ(a, b, ...)                                              \
  VELOX_CHECK((a) == (b), "Check failed: {} == {}", (a), (b))
...
#define VELOX_CHECK_GT(a, b, ...)                                              \
  VELOX_CHECK((a) > (b), "Check failed: {} > {}", (a), (b))
#define VELOX_CHECK_NULL(a, ...)                                               \
  VELOX_CHECK((a) == nullptr, "Check failed: {} == nullptr", (void *)(a))
#define VELOX_CHECK_NOT_NULL(a, ...)                                           \
  VELOX_CHECK((a) != nullptr, "Check failed: {} != nullptr", (void *)(a))
#define VELOX_DCHECK VELOX_CHECK
#define VELOX_DCHECK_LT VELOX_CHECK_LT

#define VELOX_FAIL(...)                                                        \
  ::facebook::velox::detail::veloxCheckFail(__FILE__, __LINE__, "FAIL",        \
                                            __VA_ARGS__)
