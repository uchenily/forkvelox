...
public:
  CallTypedExpr(std::shared_ptr<const Type> type,
                std::vector<TypedExprPtr> inputs, std::string name)
...
public:
  static TypedExprPtr
  inferTypes(const TypedExprPtr &untyped,
    ...
    if (auto f = std::dynamic_pointer_cast<FieldAccessTypedExpr>(untyped)) {
      ...
      for (size_t i = 0; i < names.size(); ++i) {
        if (names[i] == f->name()) {
          return std::make_shared<FieldAccessTypedExpr>(rowType->childAt(i),
                                                        f->name());
        }
      }
    ...
    if (auto lambda = std::dynamic_pointer_cast<LambdaTypedExpr>(untyped)) {
      VELOX_CHECK_NOT_NULL(
          lambdaInputTypes,
          "Lambda input types are required for type inference");
      ...
      if (rowType) {
        ...
        for (size_t i = 0; i < outerNames.size(); ++i) {
          if (std::find(scopedNames.begin(), scopedNames.end(),
                        outerNames[i]) == scopedNames.end()) {
      ...
      auto lambdaSignature =
          ROW(std::vector<std::string>(names.begin(), names.end()),
              std::vector<TypePtr>(lambdaInputTypes->begin(),
                                   lambdaInputTypes->end()));
    ...
    if (auto call = std::dynamic_pointer_cast<CallTypedExpr>(untyped)) {
      ...
      if (name == "transform" || name == "filter") {
        VELOX_CHECK_EQ(call->inputs().size(), 2, "{} expects 2 arguments",
                       name);
        ...
        VELOX_CHECK_NOT_NULL(lambdaExpr.get(), "{} expects lambda argument",
                             name);
      ...
      if (name == "plus" || name == "minus" || name == "multiply" ||
          name == "mod" || name == "divide") {
        type = BIGINT();
      } else if (name == "concat" || name == "upper" || name == "substr") {
        type = VARCHAR();
      } else if (name == "eq" || name == "neq" || name == "lt" ||
                 name == "gt" || name == "lte" || name == "gte") {
        type = INTEGER();
      ...
      return std::make_shared<CallTypedExpr>(type, std::move(typedInputs),
                                             name);
    }
