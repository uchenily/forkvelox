...
public:
  explicit FvxRowReaderAdapter(std::unique_ptr<FvxRowReader> reader) : reader_(std::move(reader)) {}
...
public:
  explicit FvxReaderAdapter(std::unique_ptr<FvxReader> reader) : reader_(std::move(reader)) {}

  std::unique_ptr<dwio::common::RowReader> createRowReader(const dwio::common::RowReaderOptions &options) override {
    return std::make_unique<FvxRowReaderAdapter>(reader_->createRowReader(options));
  }
...
public:
  std::unique_ptr<dwio::common::Reader> createReader(std::unique_ptr<dwio::common::BufferedInput> input,
    ...
    auto reader = std::make_unique<FvxReader>(input->file(), options.memoryPool());
...
void registerFvxReaderFactory() {
  dwio::common::registerReaderFactory(dwio::common::FileFormat::FVX, std::make_unique<FvxReaderFactory>());
}
