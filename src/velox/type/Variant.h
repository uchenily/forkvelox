...
public:
  ...
  Variant(int32_t val)
      : kind_(TypeKind::INTEGER), value_(static_cast<int64_t>(val)) {
  } // Promoted
  Variant(std::string val) : kind_(TypeKind::VARCHAR), value_(val) {}
  Variant(const char *val)
      : kind_(TypeKind::VARCHAR), value_(std::string(val)) {}
  Variant(TypeKind kind)
      : kind_(kind), value_(std::monostate{}) {} // Null of kind
